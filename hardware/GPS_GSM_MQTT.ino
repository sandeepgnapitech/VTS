...
const int publishInterval = 2000; // 10 seconds
...
void setup()
{
  ...
  else
  {
    ...
    else
    {
      ...
      Serial.println("Failed to connect to MQTT broker. Error: " + String(client.state()));
    }
  }
...
void loop()
{
  ...
  else
  {
    ...
    if (millis() - lastPublish >= publishInterval)
    {
      ...
      if (gps.location.isValid())
      { // Only publish if GPS data is valid
      ...
      else
      {
        ...
        // Serial.println("No valid GPS data to publish.");
      }
    }
  }
}
...
bool connectWiFi()
{
  ...
  else
  {
    ...
    case WL_IDLE_STATUS:
      Serial.println("Idle - possible chip issue");
      break;

    case WL_DISCONNECTED:
      Serial.println("Disconnected - check credentials");
      break;
...
void handleGPS()
{
  ...
  const unsigned long timeout = 1000; // 10 seconds timeout
  ...
  if (millis() - lastDataTime > timeout)
  {
    ...
    Serial.println("GPS data timeout. No data received for 10 seconds.");

    // Optionally, you can attempt to reinitialize the GPS module here
  }
}
...
void handleConnections()
{
  ...
  else if (isWiFiConnected)
  {
    ...
    if (!connectWiFi())
    { // Try to reconnect to WiFi first
...
void publishData()
{
  ...
  if (!gps.location.isValid())
  {
    ...
    Serial.println("No valid GPS data available. Skipping publish.");
  ...
  String topic = String(mqtt_topic_prefix) + credentials.deviceId + mqtt_topic_suffix;
  ...
  if (isWiFiConnected && client.connected())
  {
    ...
    Serial.println("MQTT client is connected. Attempting to publish...");
    ...
    while (retries > 0)
    {
      ...
      else
      {
        ...
        Serial.println("Failed to publish data to MQTT. Retries left: " + String(retries));
    ...
    if (retries == 0)
    {
      ...
      Serial.println("Failed to publish data after multiple retries.");
    }
  }
  else
  {

    Serial.println("No active connection to publish data.");
